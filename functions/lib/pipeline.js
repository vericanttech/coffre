...
function getDb() {
    return (0, firestore_1.getFirestore)(admin.app(), "coffre-ios");
}
...
function fallbackExtraction(language = "en") {
...
function buildExtractionPrompt(existingCustomLabels, redactedText, ocrTextShort, language = "en") {
    const lang = language === "ar" || language === "fr" ? language : "en";
    const instr = LANGUAGE_INSTRUCTIONS[lang] ?? LANGUAGE_INSTRUCTIONS.en;
    const baseList = BASE_CATEGORIES.join(", ");
    const existingList = existingCustomLabels.length > 0
...
function buildVisionExtractionPrompt(existingCustomLabels, language = "en") {
    const lang = language === "ar" || language === "fr" ? language : "en";
    const instr = LANGUAGE_INSTRUCTIONS[lang] ?? LANGUAGE_INSTRUCTIONS.en;
    const baseList = BASE_CATEGORIES.join(", ");
    const existingList = existingCustomLabels.length > 0
...
async function extractWithGeminiVision(buffer, fileType, userId, language = "en") {
    ...
    try {
        ...
        const isBaseCategory = BASE_CATEGORIES.includes(rawCategory);
        ...
        let customCategory = category === "custom"
            ? String(parsed.customCategory ?? (isBaseCategory ? undefined : rawCategory) ?? fallbackLabel).trim().slice(0, 80) || fallbackLabel
            : undefined;
        const norm = customCategory?.trim() ?? "";
        const isUnclassified = norm.toLowerCase() === "to classify" ||
...
async function extractWithGemini(redactedText, userId, ocrTextShort, language = "en") {
    if (!redactedText || redactedText.length < 10)
        return fallbackExtraction(language);
    const project = process.env.GCLOUD_PROJECT ?? process.env.GCP_PROJECT;
    const location = process.env.GOOGLE_CLOUD_LOCATION ?? "us-central1";
    if (!project) {
    ...
    try {
        ...
        functions.logger.error("pipeline: gemini extraction failed, falling back", { error: e.message });
        return fallbackExtraction(language);
    }
}
function toExtractedFields(parsed) {
...
function extractedFieldsForFirestore(fields) {
    const out = {};
    if (fields.date !== undefined)
        out.date = fields.date;
    if (fields.parties !== undefined)
        out.parties = fields.parties;
    if (fields.amounts !== undefined)
        out.amounts = fields.amounts;
    if (fields.referenceNumber !== undefined)
        out.referenceNumber = fields.referenceNumber;
    if (fields.expiryDate !== undefined)
        out.expiryDate = fields.expiryDate;
    return out;
}
/** Get existing custom category labels (customCategory) for user, to suggest reuse. */
async function getExistingCustomCategoryLabels(userId) {
...
function mergeKeywordsFromMap(keywords, redactionMap) {
...
async function runPipeline(input) {
    ...
    const { redactedText, redactionMap } = (0, redact_1.redact)(rawText);
    functions.logger.info("pipeline: redaction complete", {
        rawLength: rawText.length,
        redactedLength: redactedText.length,
    });
    let extraction;
    const useVisionFallback = (emptyOrFailed || !redactedText || redactedText.length < SHORT_OCR_CHAR_THRESHOLD) &&
        buffer != null;
